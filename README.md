...
`feishu_sessions.json` 的 `document_grouping` 为 `daily`。文档按北京时间每天一份，每位主播一个一级标题；同一主播再次上场，文字继续插到已有章节尾部。主播按排班表判断，淘宝、天猫、京东同一场直播仍映射到 `live/taobao` 的天猫排班。跨午夜的新排班按日期分开；跨交班短片段按开始时间唯一归属，可能带有交班前后声音，保留时间戳供核对。
